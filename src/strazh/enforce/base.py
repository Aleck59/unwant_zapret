...
class Enforcer(ABC):
    ...
    def path_of(self, pid: int) -> str | None:
        """Путь к файлу одного процесса.

        Отдельно от `process_list` потому, что наблюдатель получает от системы
        только номер и имя: перебирать ради одного номера весь список — та же
        расточительность, от которой ушли, отказавшись от опроса.
        """
