...
просыпается ни разу. Наблюдатель за загрузками перед обходом смотрит время
изменения папки и не читает её, если ничего не менялось, а само чтение идёт
через `os.scandir` — на Windows он отдаёт размер и время вместе со списком.
Окно перерисовывает показания, только когда они изменились.
