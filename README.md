...
| Локальная групповая политика | Те же запреты в файлах политики Windows: стереть ключ из реестра мало — система вернёт его сама. Видно в `gpedit.msc` | Не даёт запретов сверх предыдущих слоёв, применяется десятки секунд |
| Наблюдение за процессами | Единственный слой, который ловит **переименованный** файл: сверяет `OriginalFilename` из ресурса версии и подпись издателя. Работает подпиской на события, а не опросом | Срабатывает всё-таки после создания процесса |
...
## Сколько это стоит в простое

Нисколько, и это отдельная задача, а не следствие.

Наблюдатель ждёт события от Windows (`Win32_ProcessStartTrace`), а не
опрашивает список процессов: в простое его поток стоит на очереди и не
просыпается ни разу. Наблюдатель за загрузками читает папки через `os.scandir` — на Windows он
отдаёт размер и время вместе со списком, без обращения к диску на каждый
файл. Окно перерисовывает показания, только когда они изменились.

Первая версия опрашивала список процессов раз в секунду, а временные папки —
раз в три, и брала за это проценты процессора круглые сутки. Плата шла
постоянно, а польза появлялась только в момент запуска чего-нибудь.

Побочная выгода важнее самой экономии: событие приходит через миллисекунды
после создания процесса, а не через полсекунды в среднем, и нежелательная
программа не успевает показать окно.
...
в окне и в журнале. Молча работать хуже, чем обещано, он не должен.
...
Файлы локальной групповой политики сохраняются целиком перед первой правкой
и возвращаются такими, какими были. Если их не было, из них вычищаются только
наши записи: чужие политики в том же файле остаются нетронутыми, поэтому файл
и не стирается целиком.
...
Скачайте `strazh-setup-*.exe` со [страницы выпусков](../../releases) и
запустите. Установщик заведёт наблюдение с запуска системы, включит защиту и
поставит значок в меню «Пуск».

**Удаление снимает изменения в системе, а не просто стирает файлы.** Перед
удалением файлов запускается тот же откат, что и кнопка «Выключить защиту»:
снимаются перехваты запуска, правила по маске, правила брандмауэра, записи в
hosts и групповая политика. Настройки, свой каталог и карантин остаются в
`%ProgramData%\Strazh` — в карантине лежат чужие файлы, которые человек мог
захотеть вернуть, и стирать их молча неправильно.

То, что это действительно так, проверяется в сборке: программа ставится
по-настоящему, вместе с заданием и включённой защитой, потом удаляется, и
проверяется, что не осталось ни задания, ни перехватов запуска в реестре.

Кому нужна переносимая сборка — рядом лежит архив: распакуйте и запустите
`Strazh.exe`. Программа попросит права администратора: без них не завести
правила запуска в общей ветке реестра.

## Обновление

«Настройки» → «О программе» → «Проверить обновление», или командой
`strazh-cli update`. Программа спрашивает GitHub о последнем выпуске и, если
он новее, предлагает поставить.

Скачанное сверяется с контрольной суммой из того же выпуска: не сошлось —
файл удаляется и не запускается. Адрес загрузки принимается только с доменов
GitHub. И самое главное: **проверяет наличие обновления программа сама, а
ставит — только когда человек нажал.** Молча подменять себя с правами системы
она не будет — это ровно тот приём, который она запрещает другим.
...
39 записей, разложенных по одиннадцати разделам — в таблице ниже близкие
сведены в одну строку. У каждой записи есть объяснение, **почему** она в
списке, и ссылки на источники; согласиться с объяснением или нет, решает
человек.
...
strazh-cli autostart on|off|status    наблюдение с запуска системы
strazh-cli update [--install]         проверить и поставить обновление
...
| Копии файлов политики до правки | `C:\ProgramData\Strazh\backup\` |
| Сами файлы политики | `C:\Windows\System32\GroupPolicy\` |
...
Проверки и выпуск — один набор, а не два. Наборов было два, и это означало
две копии сборки, которые расходятся, и возможность выпустить то, что не
проходило проверок: выпуск начинался с чистого листа и повторял только часть
шагов. Теперь запись в Releases — последняя задача того же прогона, и
добраться до неё можно только через всё остальное.

На каждую отправку: разбор кода и типов, проверка каталога, прогон на Ubuntu
и Windows под Python 3.11 и 3.12, подъём окна на подставном экране, сборка
PyInstaller, сборка установщика и — по-настоящему — установка, проверка и
удаление на машине сборки.

Выпуск начинается тегом `v1.2.0` или кнопкой «Run workflow» с указанным
тегом; версия в теге сверяется с версией в коде. Собирается на GitHub, а не
на машине разработчика, ровно по одной причине: программа ставится с правами
системы, и тот, кто её скачивает, должен уметь проверить, из какого исходного
кода она собрана.
...
Разработчик: **Aleck59**, 2026. Лицензия MIT, см. [LICENSE](LICENSE).
